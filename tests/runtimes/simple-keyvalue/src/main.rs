...
fn main() {
    ...
    let init = |protocol: &Arc<Protocol>,
     ...
     -> Option<Box<dyn TxnDispatcher>> {
        ...
        let initializer_client = km_client.clone();
        rpc.set_keymanager_policy_update_handler(Box::new(move |raw_signed_policy| {
            km_client.set_policy(raw_signed_policy)
        }));

        txn.set_context_initializer(move |ctx: &mut TxnContext| {
            ctx.runtime = Box::new(Context {
                test_runtime_id: rt_id.clone(),
                km_client: initializer_client.clone(),
            })
        });
