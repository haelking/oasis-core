...
func (g *Genesis) SanityCheck(
	...
	stakeLedger map[staking.ID]*staking.Account,
...
func SanityCheckStake(
	...
	generatedEscrows := make(map[staking.ID]*staking.EscrowAccount)

	// Generate escrow account for all entities.
	for _, entity := range entities {
		var escrow *staking.EscrowAccount
		acctID := staking.NewIDFromPublicKey(entity.ID)
		acct, ok := accounts[acctID]
		if ok {
		...
		generatedEscrows[acctID] = escrow
	}

	for _, node := range nodes {
		// Add node stake claims.
		acctID := staking.NewIDFromPublicKey(node.EntityID)
		generatedEscrows[acctID].StakeAccumulator.AddClaimUnchecked(StakeClaimForNode(node.ID), StakeThresholdsForNode(node))
	}
	for _, rt := range runtimes {
		// Add runtime stake claims.
		acctID := staking.NewIDFromPublicKey(rt.EntityID)
		generatedEscrows[acctID].StakeAccumulator.AddClaimUnchecked(StakeClaimForRuntime(rt.ID), StakeThresholdsForRuntime(rt))
	}
	...
	for _, entity := range entities {
		...
		acctID := staking.NewIDFromPublicKey(entity.ID)
		generatedEscrow = generatedEscrows[acctID]
		acct, ok := accounts[acctID]
		if ok {
		...
		if isGenesis {
			...
			expectedClaims := generatedEscrows[acctID].StakeAccumulator.Claims
			...
			for claim, expectedThresholds := range expectedClaims {
				...
				if !ok {
					return fmt.Errorf("missing claim %s for account %s", claim, acctID)
				}
