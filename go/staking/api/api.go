...
var (
	...
	// actual ledger.
	CommonPoolAccountID = NewIDFromPublicKey(
	...
	// The ID is invalid to prevent it being accidentally used in the
	// actual ledger.
	FeeAccumulatorAccountID = NewIDFromPublicKey(
...
type Backend interface {
	...
	// or escrow balance.
	Accounts(ctx context.Context, height int64) ([]ID, error)
	...
	// (delegator).
	Delegations(ctx context.Context, query *OwnerQuery) (map[ID]*Delegation, error)

	// DebondingDelegations returns the list of debonding delegations for
	// the given owner (delegator).
	DebondingDelegations(ctx context.Context, query *OwnerQuery) (map[ID][]*DebondingDelegation, error)
...
type Escrow struct {
	Account ID         `json:"escrow_account"`
...
type Genesis struct {
	...
	Ledger map[ID]*Account `json:"ledger,omitempty"`

	Delegations          map[ID]map[ID]*Delegation            `json:"delegations,omitempty"`
	DebondingDelegations map[ID]map[ID][]*DebondingDelegation `json:"debonding_delegations,omitempty"`
}
...
type ConsensusParameters struct {
	...
	UndisableTransfersFrom map[ID]bool `json:"undisable_transfers_from,omitempty"`
