...
func AuthenticateAndPayFees(
	...
	if ctx.IsCheckOnly() {
		...
		ownAcctID := staking.NewIDFromPublicKey(ctx.AppState().OwnTxSigner())
		if !ownAcctID.Equal(&signerAcctID) {
	...
	if err := state.SetAccount(ctx, signerAcctID, account); err != nil {
